...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  constructor(public menuItems: MenuItems, private route : Router, private _service : AdminService) {
    let jwt = helper.decodeToken(localStorage.getItem('jwt'));
    this.user.id_user = jwt.ID;
    this.animateSidebar = '';
    this.navType = 'st2';
  ...
  ngOnInit() {
    this.setBackgroundPattern('dark');
    this._service.getSurveyById(this.user.id_user).then(x => {this.user =  x, localStorage.setItem("nameUser", this.user.full_name);});
  }
